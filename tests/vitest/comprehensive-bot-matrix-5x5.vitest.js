/**
 * Comprehensive 5x5 Bot Matrix - 100 Game Series
 * 
 * Jeder Bot kämpft gegen jeden anderen Bot in 100-Spiel-Serien
 * für eine vollständige Performance-Matrix
 */
...
class MatrixBotAI {
    ...
    getUniversalBestMove(game) {
        ...
        // STAGE 2: Block threats (including forks)
    ...
    selectFromSafeColumns(game, safeColumns) {
        ...
        switch (this.difficulty) {
            ...
            case 'smart-random':
                return this.selectWithSimpleStrategy(game, safeColumns);
            ...
            case 'defensive':
                return this.selectPureDefensive(game, safeColumns);
    ...
    selectWithSimpleStrategy(game, safeColumns) {
        // Smart Random: Center preference
        return this.selectCenterBiased(safeColumns);
    }
    
    selectOffensiveWeighted(game, safeColumns) {
        // Offensiv-Gemischt: Offensive potential weighted
        ...
        for (const col of safeColumns) {
            ...
            const totalScore = offensiveScore + centerBonus;
    ...
    selectDefensiveWeighted(game, safeColumns) {
        // Defensiv-Gemischt: Defensive potential weighted
        let bestCol = safeColumns[0];
        let bestScore = -1;
        ...
        for (const col of safeColumns) {
            ...
            const centerBonus = (3 - Math.abs(col - 3)) * 0.1;
            const totalScore = defensiveScore + centerBonus;
    ...
    selectEnhancedStrategy(game, safeColumns) {
        // Enhanced Smart: Balanced approach
        let bestCol = safeColumns[0];
        let bestScore = -1;
    ...
    selectPureDefensive(game, safeColumns) {
        // Defensive: Maximum defensive value
        let bestCol = safeColumns[0];
        let bestScore = -1;
        ...
        for (const col of safeColumns) {
            const defensiveScore = this.evaluateDefensivePotential(game, col);
            const offensiveScore = this.evaluateOffensivePotential(game, col);
            const totalScore = defensiveScore * 2 + offensiveScore * 0.5;
    ...
    evaluateOffensivePotential(game, col) {
        // Simple offensive potential calculation
        let row = game.ROWS - 1;
...
describe('Comprehensive 5x5 Bot Matrix - 100 Game Series', () => {
    test('should run complete 5x5 matrix and generate performance table', () => {
        const bots = [
            'easy',
            'smart-random', 
            'offensiv-gemischt',
        ...
        console.log('\n🎮 Starting 5x5 Bot Matrix (25 matches × 100 games = 2500 total games)...\n');
