        ...
        for (let row = 0; row < this.game.BOARD_SIZE; row++) {
            for (let col = 0; col < this.game.BOARD_SIZE; col++) {
                if (this.game.board[row][col] === this.game.EMPTY) {
                    ...
                    this.game.board[row][col] = currentPlayer;
                    
                    // Check if this completes a 5-in-a-row
                    if (this.checkWinAt(row, col, currentPlayer)) {
        ...
        for (let row = 0; row < this.game.BOARD_SIZE; row++) {
            for (let col = 0; col < this.game.BOARD_SIZE; col++) {
                if (this.game.board[row][col] === this.game.EMPTY) {
                    ...
                    if (this.checkWinAt(row, col, opponent)) {
            ...
            for (const [dr, dc] of this.directions) {
                for (let dist = 1; dist <= 2; dist++) {
                    ...
                    if (this.isValidPosition(checkRow, checkCol) && 
                        this.game.board[checkRow][checkCol] === this.game.EMPTY) {
                        
                        // Temporarily place opponent piece
                        this.game.board[checkRow][checkCol] = opponent;
                        
                        // Check if opponent gets 4-in-a-row
                        if (this.count4InARow(checkRow, checkCol, opponent) > 0) {
                            createsThreat = true;
                        }
                        
                        // Remove temporary opponent piece
                        this.game.board[checkRow][checkCol] = this.game.EMPTY;
        ...
        for (const [dr, dc] of this.directions) {
            ...
            for (let i = 1; i < 5; i++) {
                ...
                if (!this.isValidPosition(newRow, newCol) || 
                    this.game.board[newRow][newCol] !== player) {
                    break;
                }
                count++;
            }
                ...
                if (this.game.board[row][col] !== this.game.EMPTY) {
                    ...
                    for (let dr = -radius; dr <= radius; dr++) {
                        for (let dc = -radius; dc <= radius; dc++) {
                            ...
                            if (this.isValidPosition(newRow, newCol) && 
                                this.game.board[newRow][newCol] === this.game.EMPTY &&
                                !considered.has(key)) {
