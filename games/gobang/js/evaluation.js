...
class GobangEvaluation {
    ...
    getWeightedRandomMove(game) {
        ...
        for (const eval of evaluations) {
            ...
            for (let i = 0; i < weight; i++) {
                weightedList.push({ row: eval.row, col: eval.col });
            }
        }
        
        console.log('🎯 Gobang weighted selection:');
        evaluations.slice(0, 10).forEach(eval => { // Show top 10
            const weight = Math.max(1, eval.score);
            console.log(`  (${eval.row + 1}, ${String.fromCharCode(65 + eval.col)}): Score ${eval.score} → Weight ${weight}`);
        });
    ...
    debugEvaluations(game, topN = 10) {
        ...
        for (let i = 0; i < Math.min(topN, evaluations.length); i++) {
            ...
            const coord = `(${eval.row + 1}, ${String.fromCharCode(65 + eval.col)})`;
            console.log(`${i + 1}. ${coord}: Score ${eval.score}`);
        }
