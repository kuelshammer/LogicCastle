...
class _Connect4AI {
    ...
    minimax(board, depth, isMaximizing, alpha, beta, game) {
        // Terminal conditions
        const _gameState = this.evaluateBoard(board, game);

        if (depth === 0 || Math.abs(_gameState) === 1000000) {
            return _gameState;
        }
     ...
     */
    findWinningMoveForPlayer(game, _player) {
        const validMoves = game.getValidMoves();
     ...
     */
    selectSmartRandomFromSafe(game, safeColumns, _helpers) {
