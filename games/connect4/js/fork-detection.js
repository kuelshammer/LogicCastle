...
class Connect4ForkDetection {
  ...
  /**
     ...
     */
  evaluateVerticalTrapPotential(_row, _col, _player) {
    let potential = 0;

    // Check horizontal connections from this column
    for (let checkRow = _row; checkRow < this.game.ROWS; checkRow++) {
      // Check left and right for potential horizontal threats
      const leftConnections = this.countHorizontalConnections(checkRow, _col, _player, -1);
      const rightConnections = this.countHorizontalConnections(checkRow, _col, _player, 1);
  ...
  getCriticalForkCounters(player) {
    const _opponent = player === this.PLAYER1 ? this.PLAYER2 : this.PLAYER1;
    const opponentForks = this.detectForkSetups(_opponent);
